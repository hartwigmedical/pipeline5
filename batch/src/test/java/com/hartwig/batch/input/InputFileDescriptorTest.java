...
public class InputFileDescriptorTest {
    ...
    public void shouldAddProtocolIfLeftOutOfFilenames() {
        assertCommandForm(builder.value(format("gs://%s", remote)).build().toCommandForm(local), remote);
    }

    @Test
    public void shouldNotDoubleAddProtocolIfIncludedInFilenames() {
        assertCommandForm(builder.value(remote).build().toCommandForm(local), remote);
    }

    @Test
    public void shouldSetName() {
        assertThat(builder.value(remote).build().name()).isEqualTo(name);
    }

...
public class InputBundleTest {
    ...
    public void setup() {
        key = "my_key";
        InputFileDescriptor another = InputFileDescriptor.builder().name("some other key").value("irrelevant")
                .billedProject("project").build();
        descriptor = InputFileDescriptor.builder().name(key).value("irrelevant")
                .billedProject("project").build();

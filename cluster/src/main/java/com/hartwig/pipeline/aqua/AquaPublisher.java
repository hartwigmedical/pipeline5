...
import com.hartwig.events.pubsub.NoopEventPublisher;
...
public class AquaPublisher {
    ...
    public AquaPublisher(final EventPublisher<AquaEvent> publisher, final String barcode, final boolean isShallow,
    ...
    public static AquaPublisher create(Arguments arguments, SomaticRunMetadata somaticRunMetadata) throws IOException {
        ...
        EventPublisher<AquaEvent> publisher;
        if (!arguments.publishToAqua()) {
            publisher = new NoopEventPublisher<>();
        } else {
            publisher = new PubsubEventBuilder().newPublisher(project, new AquaEvent.EventDescriptor());
        }

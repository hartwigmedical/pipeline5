...
public class StructuralCaller {
    ...
    public StructuralCallerOutput run(AlignmentPair pair, BamMetricsOutput metricsOutput, BamMetricsOutput mateMetricsOutput) {
        ...
        ResourceDownload referenceGenomeDownload = ResourceDownload.from(runtimeBucket,
                new Resource(storage, arguments.resourceBucket(), ResourceNames.REFERENCE_GENOME));
        ...
        String gridssWorkingDirForReferenceBam = format("%s/%s.gridss.working", VmDirectories.OUTPUT, basename(referenceBam.getLocalTargetPath()));
        ...
        String preprocessSvOutputReferenceBam = format("%s/%s.sv.bam", gridssWorkingDirForReferenceBam, basename(referenceBam.getLocalTargetPath()));
        String preprocessSvOutputTumorBam = format("%s/%s.sv.bam", gridssWorkingDirForTumorBam, basename(tumorBam.getLocalTargetPath()));

        Preprocess.PreprocessResult preprocessedSample = new Preprocess(commandFactory)
                .initialise(referenceBam.getLocalTargetPath(), referenceSampleName, referenceGenomePath, metricsDownload.getLocalTargetPath(), preprocessSvOutputReferenceBam);
        Preprocess.PreprocessResult preprocessedTumor = new Preprocess(commandFactory)
                .initialise(tumorBam.getLocalTargetPath(), tumorSampleName, referenceGenomePath, mateMetricsDownload.getLocalTargetPath(), preprocessSvOutputTumorBam);
        ...
        IdentifyVariants calling = commandFactory.buildIdentifyVariants(referenceBam.getLocalTargetPath(),
                tumorBam.getLocalTargetPath(), assemblyResult.assemblyBam(), referenceGenomePath, GridssCommon.blacklist());

        Annotation.AnnotationResult annotationResult = new Annotation(commandFactory).initialise(referenceBam.getLocalTargetPath(),
                tumorBam.getLocalTargetPath(), assemblyResult.assemblyBam(), calling.resultantVcf(), referenceGenomePath);
        ...
        return StructuralCallerOutput.builder()
                .status(status)
                .maybeStructuralVcf(GoogleStorageLocation.of(runtimeBucket.name(),
                        resultsDirectory.path()))
                .maybeSvRecoveryVcf(GoogleStorageLocation.of(runtimeBucket.name(),
                        resultsDirectory.path()))
                .build();
    }

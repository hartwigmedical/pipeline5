...
function die() {
  ...
  gsutil -m cp /var/log/run.log gs://run-colo829v003t-qntrc/flagstat
  echo $exit_code > /tmp/JOB_FAILURE
  gsutil -m cp /tmp/JOB_FAILURE gs://run-colo829v003t-qntrc/flagstat
  exit $exit_code
}
...
echo $(date "+%Y-%m-%d %H:%M:%S") "Running command SubShellCommand with bash: (/opt/tools/sambamba/0.6.8/sambamba flagstat -t $(grep -c '^processor' /proc/cpuinfo) /data/input/COLO829v003T.bam > /data/output/COLO829v003T.flagstat)" >>/var/log/run.log 2>&1 || die
(/opt/tools/sambamba/0.6.8/sambamba flagstat -t $(grep -c '^processor' /proc/cpuinfo) /data/input/COLO829v003T.bam > /data/output/COLO829v003T.flagstat) >>/var/log/run.log 2>&1 || die
echo $(date "+%Y-%m-%d %H:%M:%S") "Running command OutputUpload with bash: (cp /var/log/run.log /data/output && gsutil -qm rsync -r /data/output/ gs://run-colo829v003t-qntrc/flagstat/results)" >>/var/log/run.log 2>&1 || die
(cp /var/log/run.log /data/output && gsutil -qm rsync -r /data/output/ gs://run-colo829v003t-qntrc/flagstat/results) >>/var/log/run.log 2>&1 || die
(echo 0 > /tmp/JOB_SUCCESS && gsutil cp /tmp/JOB_SUCCESS gs://run-colo829v003t-qntrc/flagstat) >>/var/log/run.log 2>&1 || die
...
function die() {
  ...
  gsutil -m cp /var/log/run.log gs://run-colo829v003t-qdvca/snp_genotype
  echo $exit_code > /tmp/JOB_FAILURE
  gsutil -m cp /tmp/JOB_FAILURE gs://run-colo829v003t-qdvca/snp_genotype
  exit $exit_code
}
...
echo $(date "+%Y-%m-%d %H:%M:%S") "Running command SnpGenotypeCommand with bash: /usr/lib/jvm/java-8-openjdk-amd64/jre/bin/java -Xmx20G -jar /opt/tools/gatk/3.8.0/GenomeAnalysisTK.jar -T UnifiedGenotyper -nct $(grep -c '^processor' /proc/cpuinfo) --input_file /data/input/COLO829v003T.bam -o /data/output/snp_genotype_output.vcf -L /opt/resources/genotype_snps/37/26SNPtaq.vcf --reference_sequence /opt/resources/reference_genome/37/Homo_sapiens.GRCh37.GATK.illumina.fasta --output_mode EMIT_ALL_SITES" >>/var/log/run.log 2>&1 || die
/usr/lib/jvm/java-8-openjdk-amd64/jre/bin/java -Xmx20G -jar /opt/tools/gatk/3.8.0/GenomeAnalysisTK.jar -T UnifiedGenotyper -nct $(grep -c '^processor' /proc/cpuinfo) --input_file /data/input/COLO829v003T.bam -o /data/output/snp_genotype_output.vcf -L /opt/resources/genotype_snps/37/26SNPtaq.vcf --reference_sequence /opt/resources/reference_genome/37/Homo_sapiens.GRCh37.GATK.illumina.fasta --output_mode EMIT_ALL_SITES >>/var/log/run.log 2>&1 || die
echo $(date "+%Y-%m-%d %H:%M:%S") "Running command OutputUpload with bash: (cp /var/log/run.log /data/output && gsutil -qm rsync -r /data/output/ gs://run-colo829v003t-qdvca/snp_genotype/results)" >>/var/log/run.log 2>&1 || die
(cp /var/log/run.log /data/output && gsutil -qm rsync -r /data/output/ gs://run-colo829v003t-qdvca/snp_genotype/results) >>/var/log/run.log 2>&1 || die
(echo 0 > /tmp/JOB_SUCCESS && gsutil cp /tmp/JOB_SUCCESS gs://run-colo829v003t-qdvca/snp_genotype) >>/var/log/run.log 2>&1 || die
...
function die() {
  ...
  gsutil -m cp /var/log/run.log gs://run-colo829v003t-qntrc/snp_genotype
  echo $exit_code > /tmp/JOB_FAILURE
  gsutil -m cp /tmp/JOB_FAILURE gs://run-colo829v003t-qntrc/snp_genotype
  exit $exit_code
}
...
echo $(date "+%Y-%m-%d %H:%M:%S") "Running command OutputUpload with bash: (cp /var/log/run.log /data/output && gsutil -qm rsync -r /data/output/ gs://run-colo829v003t-qntrc/snp_genotype/results)" >>/var/log/run.log 2>&1 || die
(cp /var/log/run.log /data/output && gsutil -qm rsync -r /data/output/ gs://run-colo829v003t-qntrc/snp_genotype/results) >>/var/log/run.log 2>&1 || die
(echo 0 > /tmp/JOB_SUCCESS && gsutil cp /tmp/JOB_SUCCESS gs://run-colo829v003t-qntrc/snp_genotype) >>/var/log/run.log 2>&1 || die
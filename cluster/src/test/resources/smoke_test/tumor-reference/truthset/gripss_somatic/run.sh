...
function die() {
  ...
  gsutil -m cp /var/log/run.log gs://run-colo829v003r-colo829v003t-qntrc/gripss_somatic
  echo $exit_code > /tmp/JOB_FAILURE
  gsutil -m cp /tmp/JOB_FAILURE gs://run-colo829v003r-colo829v003t-qntrc/gripss_somatic
  exit $exit_code
}
...
echo $(date "+%Y-%m-%d %H:%M:%S") "Running command JavaJarCommand with bash: java -Xmx16G -jar /opt/tools/gripss/2.0/gripss.jar -ref_genome /opt/resources/reference_genome/37/Homo_sapiens.GRCh37.GATK.illumina.fasta -known_hotspot_file /opt/resources/fusions/37/known_fusions.37.bedpe -pon_sgl_file /opt/resources/gridss_pon/37/gridss_pon_single_breakend.37.bed -pon_sv_file /opt/resources/gridss_pon/37/gridss_pon_breakpoint.37.bedpe -output_id somatic -sample COLO829v003T -reference COLO829v003R -vcf /data/input/COLO829v003T.gridss.unfiltered.vcf.gz -output_dir /data/output" >>/var/log/run.log 2>&1 || die
java -Xmx16G -jar /opt/tools/gripss/2.0/gripss.jar -ref_genome /opt/resources/reference_genome/37/Homo_sapiens.GRCh37.GATK.illumina.fasta -known_hotspot_file /opt/resources/fusions/37/known_fusions.37.bedpe -pon_sgl_file /opt/resources/gridss_pon/37/gridss_pon_single_breakend.37.bed -pon_sv_file /opt/resources/gridss_pon/37/gridss_pon_breakpoint.37.bedpe -output_id somatic -sample COLO829v003T -reference COLO829v003R -vcf /data/input/COLO829v003T.gridss.unfiltered.vcf.gz -output_dir /data/output >>/var/log/run.log 2>&1 || die
echo $(date "+%Y-%m-%d %H:%M:%S") "Running command OutputUpload with bash: (cp /var/log/run.log /data/output && gsutil -qm rsync -r /data/output/ gs://run-colo829v003r-colo829v003t-qntrc/gripss_somatic/results)" >>/var/log/run.log 2>&1 || die
(cp /var/log/run.log /data/output && gsutil -qm rsync -r /data/output/ gs://run-colo829v003r-colo829v003t-qntrc/gripss_somatic/results) >>/var/log/run.log 2>&1 || die
(echo 0 > /tmp/JOB_SUCCESS && gsutil cp /tmp/JOB_SUCCESS gs://run-colo829v003r-colo829v003t-qntrc/gripss_somatic) >>/var/log/run.log 2>&1 || die
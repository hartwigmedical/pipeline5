...
public class SageSomaticCallerTest extends TertiaryStageTest<SageOutput> {
    ...
    protected List<String> expectedCommands() {
        return ImmutableList.of("unzip -d /opt/resources /opt/resources/snpeff/37/snpEff_v4_3_GRCh37.75.zip",
                "java -Xmx110G -cp /opt/tools/sage/2.6/sage.jar com.hartwig.hmftools.sage.SageApplication -tumor tumor -tumor_bam /data/input/tumor.bam -reference reference -reference_bam /data/input/reference.bam -hotspots /opt/resources/sage/37/KnownHotspots.somatic.hg19.vcf.gz -panel_bed /opt/resources/sage/37/ActionableCodingPanel.somatic.hg19.bed.gz -high_confidence_bed /opt/resources/GIAB_high_conf/37/NA12878_GIAB_highconf_IllFB-IllGATKHC-CG-Ion-Solid_ALLCHROM_v3.2.2_highconf.bed.gz -ref_genome /opt/resources/reference_genome/37/Homo_sapiens.GRCh37.GATK.illumina.fasta -out /data/output/tumor.sage.somatic.vcf.gz -assembly hg19 -threads $(grep -c '^processor' /proc/cpuinfo) -coverage_bed /opt/resources/sage/37/ActionableCodingPanel.somatic.hg19.bed.gz",

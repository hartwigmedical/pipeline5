...
public class SomaticCallerTest extends TertiaryStageTest<SomaticCallerOutput> {
    ...
    protected List<String> expectedCommands() {
        return ImmutableList.of(
                "java -Xmx8G -cp /opt/tools/sage/1.1/sage.jar com.hartwig.hmftools.sage.SageHotspotApplication -tumor tumor -tumor_bam /data/input/tumor.bam -reference reference -reference_bam /data/input/reference.bam -known_hotspots /data/resources/hotspots.tsv -coding_regions /data/resources/coding_regions.bed -ref_genome /data/resources/reference.fasta -out /data/output/tumor.sage.hotspots.vcf.gz",
                ...
                "/opt/tools/bcftools/1.3.1/bcftools annotate -a /data/resources/SAGE_PON.vcf.gz -c SAGE_PON_COUNT -o /data/output/tumor.sage.hotspots.pon.annotated.vcf.gz -O z /data/output/tumor.sage.hotspots.filtered.vcf.gz",
                ...
                "/opt/tools/strelka/1.0.14/bin/configureStrelkaWorkflow.pl --tumor /data/input/tumor.bam --normal /data/input/reference.bam --config /data/resources/strelka.ini --ref /data/resources/reference.fasta --output-dir /data/output/strelkaAnalysis",
                ...
                "/opt/tools/bcftools/1.3.1/bcftools annotate -a /data/resources/mappability.bed.gz -h /data/resources/mappability.hdr -c CHROM,FROM,TO,-,MAPPABILITY -o /data/output/tumor.mappability.annotated.vcf.gz -O z /data/output/tumor.strelka.vcf",
                "/opt/tools/tabix/0.2.6/tabix /data/output/tumor.mappability.annotated.vcf.gz -p vcf",
                "/opt/tools/bcftools/1.3.1/bcftools annotate -a /data/resources/GERMLINE_PON.vcf.gz -c GERMLINE_PON_COUNT -o /data/output/tumor.germline.pon.annotated.vcf.gz -O z /data/output/tumor.mappability.annotated.vcf.gz",
                "/opt/tools/tabix/0.2.6/tabix /data/output/tumor.germline.pon.annotated.vcf.gz -p vcf",
                "/opt/tools/bcftools/1.3.1/bcftools annotate -a /data/resources/SOMATIC_PON.vcf.gz -c SOMATIC_PON_COUNT -o /data/output/tumor.somatic.pon.annotated.vcf.gz -O z /data/output/tumor.germline.pon.annotated.vcf.gz",
                "/opt/tools/tabix/0.2.6/tabix /data/output/tumor.somatic.pon.annotated.vcf.gz -p vcf",
                "java -Xmx20G -jar /opt/tools/strelka-post-process/1.4/strelka-post-process.jar -v /data/output/tumor.somatic.pon.annotated.vcf.gz -hc_bed /data/resources/strelka-post-process.bed -t tumor -o /data/output/tumor.strelka.post.processed.vcf.gz -b /data/input/tumor.bam",
                ...
                "java -Xmx8G -cp /opt/tools/sage/1.1/sage.jar com.hartwig.hmftools.sage.SageHotspotAnnotation -source_vcf /data/output/tumor.pon.filtered.vcf.gz -hotspot_vcf /data/output/tumor.sage.pon.filter.vcf.gz -known_hotspots /data/resources/hotspots.tsv -out /data/output/tumor.sage.hotspots.annotated.vcf.gz",
                "/opt/tools/snpEff/4.3s/snpEff.sh /opt/tools/snpEff/4.3s/snpEff.jar /data/resources/snpeff.config GRCh37.75 /data/output/tumor.sage.hotspots.annotated.vcf.gz /data/output/tumor.snpeff.annotated.vcf",
                ...
                "/opt/tools/bcftools/1.3.1/bcftools annotate -a /data/resources/dbsnp.vcf.gz -c ID -o /data/output/tumor.dbsnp.annotated.vcf.gz -O z /data/output/tumor.snpeff.annotated.vcf.gz",
                "/opt/tools/tabix/0.2.6/tabix /data/output/tumor.dbsnp.annotated.vcf.gz -p vcf",
                "/opt/tools/bcftools/1.3.1/bcftools annotate -a /data/resources/cosmic_collapsed.vcf.gz -c ID,INFO -o /data/output/tumor.cosmic.annotated.final.vcf.gz -O z /data/output/tumor.dbsnp.annotated.vcf.gz",

...
public class SomaticCallerTest extends TertiaryStageTest<SomaticCallerOutput> {
    ...
    protected List<String> expectedCommands() {
        return ImmutableList.of(
                "java -Xmx8G -cp /opt/tools/sage/2.0/sage.jar com.hartwig.hmftools.sage.SageHotspotApplication -tumor tumor -tumor_bam /data/input/tumor.bam -reference reference -reference_bam /data/input/reference.bam -known_hotspots /opt/resources/sage/KnownHotspots.tsv -coding_regions /opt/resources/sage/CodingRegions.bed -ref_genome /opt/resources/reference_genome/Homo_sapiens.GRCh37.GATK.illumina.fasta -out /data/output/tumor.sage.hotspots.vcf.gz",
                ...
                "(/opt/tools/bcftools/1.3.1/bcftools annotate -a /opt/resources/mappability/out_150_hg19.mappability.bed.gz -h /opt/resources/mappability/mappability.hdr -c CHROM,FROM,TO,-,MAPPABILITY /data/output/tumor.strelka.vcf -O z -o /data/output/tumor.mappability.annotated.vcf.gz)",
                ...
                "java -Xmx8G -cp /opt/tools/sage/2.0/sage.jar com.hartwig.hmftools.sage.SageHotspotAnnotation -source_vcf /data/output/tumor.pon.filtered.vcf.gz -hotspot_vcf /data/output/tumor.sage.pon.filter.vcf.gz -known_hotspots /opt/resources/sage/KnownHotspots.tsv -out /data/output/tumor.sage.hotspots.annotated.vcf.gz",
                ...
                "(/opt/tools/bcftools/1.3.1/bcftools annotate -a /opt/resources/dbsnps/dbsnp_137.b37.vcf.gz -c ID /data/output/tumor.snpeff.annotated.vcf.gz -O z -o /data/output/tumor.dbsnp.annotated.vcf.gz)",
                "/opt/tools/tabix/0.2.6/tabix /data/output/tumor.dbsnp.annotated.vcf.gz -p vcf",
                "(/opt/tools/bcftools/1.3.1/bcftools annotate -a /opt/resources/cosmic_v85/CosmicCodingMuts_v85_collapsed.vcf.gz -c ID,INFO /data/output/tumor.dbsnp.annotated.vcf.gz -O z -o /data/output/tumor.cosmic.annotated.final.vcf.gz)",

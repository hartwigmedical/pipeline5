...
public class SageGermlineCallerTest extends TertiaryStageTest<SageOutput> {
    ...
    protected Stage<SageOutput, SomaticRunMetadata> createVictim() {
        return new SageGermlineCaller(TestInputs.defaultPair(), TestInputs.HG19_RESOURCE_FILES, new NoopPersistedDataset());
    }

    @Override
    protected List<String> expectedCommands() {
        return ImmutableList.of(
                "unzip -d /opt/resources /opt/resources/snpeff/hg19/snpEff_v4_3_GRCh37.75.zip",
                "java -Xmx110G -cp /opt/tools/sage/2.5/sage.jar com.hartwig.hmftools.sage.SageApplication -tumor reference -tumor_bam /data/input/reference.bam -reference tumor -reference_bam /data/input/tumor.bam -hotspots /opt/resources/sage/hg19/KnownHotspots.germline.hg19.vcf.gz -panel_bed /opt/resources/sage/hg19/ActionableCodingPanel.germline.hg19.bed.gz -hotspot_min_tumor_qual 50 -panel_min_tumor_qual 75 -hotspot_max_germline_vaf 100 -hotspot_max_germline_rel_raw_base_qual 100 -panel_max_germline_vaf 100 -panel_max_germline_rel_raw_base_qual 100 -mnv_filter_enabled false -high_confidence_bed /opt/resources/GIAB_high_conf/hg19/NA12878_GIAB_highconf_IllFB-IllGATKHC-CG-Ion-Solid_ALLCHROM_v3.2.2_highconf.bed.gz -ref_genome /opt/resources/reference_genome/hg19/Homo_sapiens.GRCh37.GATK.illumina.fasta -out /data/output/tumor.sage.germline.vcf.gz -assembly hg19 -threads $(grep -c '^processor' /proc/cpuinfo) -panel_only",

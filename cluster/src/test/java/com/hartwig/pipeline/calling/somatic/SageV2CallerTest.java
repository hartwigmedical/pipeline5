...
public class SageV2CallerTest extends TertiaryStageTest<SomaticCallerOutput> {
    ...
    protected List<String> expectedCommands() {
        return ImmutableList.of("unzip -d /opt/resources /opt/resources/snpeff/hg37/snpEff_v4_3_GRCh37.75.zip",
                "java -Xmx110G -cp /opt/tools/sage/2.1/sage.jar com.hartwig.hmftools.sage.SageApplication -tumor tumor -tumor_bam /data/input/tumor.bam -reference reference -reference_bam /data/input/reference.bam -hotspots /opt/resources/sage/hg37/KnownHotspots.hg19.vcf.gz -panel_bed /opt/resources/sage/hg37/ActionableCodingPanel.hg19.bed.gz -high_confidence_bed /opt/resources/beds/hg37/NA12878_GIAB_highconf_IllFB-IllGATKHC-CG-Ion-Solid_ALLCHROM_v3.2.2_highconf.bed.gz -ref_genome /opt/resources/reference_genome/hg37/Homo_sapiens.GRCh37.GATK.illumina.fasta -out /data/output/tumor.sage.somatic.vcf.gz -threads $(grep -c '^processor' /proc/cpuinfo)",
                "(/opt/tools/bcftools/1.9/bcftools filter -i 'FILTER=\"PASS\"' /data/output/tumor.sage.somatic.vcf.gz -O u | /opt/tools/bcftools/1.9/bcftools view -s tumor -O z -o /data/output/tumor.sage.pass.vcf.gz)",
                ...
                "(/opt/tools/bcftools/1.9/bcftools annotate -a /opt/resources/sage/hg37/SageGermlinePon.hg19.vcf.gz -c PON_COUNT /data/output/tumor.mappability.annotated.vcf.gz -O z -o /data/output/tumor.sage.pon.annotated.vcf.gz)",
                "/opt/tools/tabix/0.2.6/tabix /data/output/tumor.sage.pon.annotated.vcf.gz -p vcf",
                "(/opt/tools/bcftools/1.9/bcftools filter -e 'PON_COUNT!= \".\" && (MIN(PON_COUNT) > 9 || (MIN(PON_COUNT) > 2 && INFO/TIER!=\"HOTSPOT\"))' -s SAGE_PON -m+ /data/output/tumor.sage.pon.annotated.vcf.gz -O z -o /data/output/tumor.sage.pon.filter.vcf.gz)",
                ...
                "java -Xmx8G -cp /opt/tools/sage/2.1/sage.jar com.hartwig.hmftools.sage.SagePostProcessApplication -in /data/output/tumor.snpeff.annotated.vcf.gz -out /data/output/tumor.sage.post.processed.vcf.gz -assembly hg19",
                "(/opt/tools/bcftools/1.9/bcftools annotate -a /opt/resources/cosmic_v85/CosmicCodingMuts_v85_collapsed.vcf.gz -c ID,INFO /data/output/tumor.sage.post.processed.vcf.gz -O z -o /data/output/tumor.cosmic.annotated.final.vcf.gz)",

...
public class GermlineCallerTest extends StageTest<GermlineCallerOutput, SingleSampleRunMetadata> {
    ...
    protected Stage<GermlineCallerOutput, SingleSampleRunMetadata> createVictim() {
        return new GermlineCaller(TestInputs.referenceAlignmentOutput(), TestInputs.REG_GENOME_37_RESOURCE_FILES, persistedDataset);
    }
    ...
    protected List<String> expectedCommands() {
        return ImmutableList.<String>builder().add("unzip -d /opt/resources /opt/resources/snpeff/37/snpEff_v4_3_GRCh37.75.zip",
                ...
                "java -Xmx29G -jar /opt/tools/gatk/3.8.0/GenomeAnalysisTK.jar -T GenotypeGVCFs -V /data/output/reference.raw_germline_caller.vcf.gz -R /opt/resources/reference_genome/37/Homo_sapiens.GRCh37.GATK.illumina.fasta -o /data/output/reference.genotype_vcfs.vcf",
                "java -Xmx29G -jar /opt/tools/gatk/3.8.0/GenomeAnalysisTK.jar -T SelectVariants -selectType SNP -selectType NO_VARIATION -R /opt/resources/reference_genome/37/Homo_sapiens.GRCh37.GATK.illumina.fasta -V /data/output/reference.genotype_vcfs.vcf -o /data/output/reference.raw_snp.vcf",
                ...
                "/opt/tools/snpEff/4.3s/snpEff.sh /opt/tools/snpEff/4.3s/snpEff.jar /opt/resources/snpeff/37/snpEff.config GRCh37.75 /data/output/reference.filtered_variants.vcf /data/output/reference.snpeff.annotated.vcf",

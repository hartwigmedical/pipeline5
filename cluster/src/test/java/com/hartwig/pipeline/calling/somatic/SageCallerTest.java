...
public class SageCallerTest extends TertiaryStageTest<SomaticCallerOutput> {
    ...
    protected Stage<SomaticCallerOutput, SomaticRunMetadata> createVictim() {
        return new SageCaller(TestInputs.defaultPair(), TestInputs.REG_GENOME_37_RESOURCE_FILES, new NoopPersistedDataset());
    }

    @Override
    protected List<String> expectedCommands() {
        return ImmutableList.of("unzip -d /opt/resources /opt/resources/snpeff/37/snpEff_v4_3_GRCh37.75.zip",
                "java -Xmx110G -cp /opt/tools/sage/2.4/sage.jar com.hartwig.hmftools.sage.SageApplication -tumor tumor -tumor_bam /data/input/tumor.bam -reference reference -reference_bam /data/input/reference.bam -hotspots /opt/resources/sage/37/KnownHotspots.somatic.hg19.vcf.gz -panel_bed /opt/resources/sage/37/ActionableCodingPanel.somatic.hg19.bed.gz -high_confidence_bed /opt/resources/GIAB_high_conf/37/NA12878_GIAB_highconf_IllFB-IllGATKHC-CG-Ion-Solid_ALLCHROM_v3.2.2_highconf.bed.gz -ref_genome /opt/resources/reference_genome/37/Homo_sapiens.GRCh37.GATK.illumina.fasta -out /data/output/tumor.sage.somatic.vcf.gz -assembly hg19 -threads $(grep -c '^processor' /proc/cpuinfo)",
                ...
                "(/opt/tools/bcftools/1.9/bcftools annotate -a /opt/resources/mappability/37/out_150_hg19.mappability.bed.gz -h /opt/resources/mappability/mappability.hdr -c CHROM,FROM,TO,-,MAPPABILITY /data/output/tumor.sage.pass.vcf.gz -O z -o /data/output/tumor.mappability.annotated.vcf.gz)",
                "/opt/tools/tabix/0.2.6/tabix /data/output/tumor.mappability.annotated.vcf.gz -p vcf",
                "(/opt/tools/bcftools/1.9/bcftools annotate -a /opt/resources/sage/37/SageGermlinePon.hg19.1000x.vcf.gz -c PON_COUNT,PON_MAX /data/output/tumor.mappability.annotated.vcf.gz -O z -o /data/output/tumor.sage.pon.annotated.vcf.gz)",
                ...
                "/opt/tools/snpEff/4.3s/snpEff.sh /opt/tools/snpEff/4.3s/snpEff.jar /opt/resources/snpeff/37/snpEff.config GRCh37.75 /data/output/tumor.sage.pon.filter.vcf.gz /data/output/tumor.sage.somatic.filtered.vcf",

...
public class StructuralCallerTest extends StageTest<StructuralCallerOutput, SomaticRunMetadata> {
    ...
    protected List<String> expectedCommands() {
        return ImmutableList.of("export PATH=\"${PATH}:/opt/tools/bwa/0.7.17\"",
                ...
                "/opt/tools/gridss/2.8.3/gridss.sh -o /data/output/tumor.gridss.unfiltered.vcf.gz -a /data/output/tumor.assembly.bam -w /data/output -r /opt/resources/reference_genome/hg37/Homo_sapiens.GRCh37.GATK.illumina.fasta -j /opt/tools/gridss/2.8.3/gridss.jar -b /opt/resources/gridss_repeatmasker_db/hg37/ENCFF001TDO.hg37.bed -c /opt/resources/gridss_config/gridss.properties /data/input/reference.bam /data/input/tumor.bam",
                "/opt/tools/tabix/0.2.6/tabix /data/output/tumor.gridss.unfiltered.vcf.gz -p vcf",
                "/bin/bash -e /opt/tools/gridss/2.8.3/failsafe_repeatmasker_invoker.sh /data/output/tumor.gridss.unfiltered.vcf.gz /data/output/tumor.repeatmasker_annotation.vcf.gz /opt/resources/gridss_repeatmasker_db/hg37/hg19.fa.out /opt/tools/gridss/2.8.3",
                ...
                "java -Xmx8G -Dsamjdk.create_index=true -Dsamjdk.use_async_io_read_samtools=true -Dsamjdk.use_async_io_write_samtools=true -Dsamjdk.use_async_io_write_tribble=true -Dsamjdk.buffer_size=4194304 -cp /opt/tools/gridss/2.8.3/gridss.jar gridss.AnnotateUntemplatedSequence REFERENCE_SEQUENCE=/opt/resources/virus_reference_genome/human_virus.fa INPUT=/data/output/tumor.repeatmasker_annotation.missingbealn.vcf OUTPUT=/data/output/tumor.repeatmasker_annotation.withannotation.vcf",
                "java -Xmx32G -Dsamjdk.create_index=true -Dsamjdk.use_async_io_read_samtools=true -Dsamjdk.use_async_io_write_samtools=true -Dsamjdk.use_async_io_write_tribble=true -Dsamjdk.buffer_size=4194304 -cp /opt/tools/gridss/2.8.3/gridss.jar picard.cmdline.PicardCommandLine SortVcf I=/data/output/tumor.repeatmasker_annotation.withbealn.vcf I=/data/output/tumor.repeatmasker_annotation.withannotation.vcf O=/data/output/tumor.viral_annotation.vcf.gz",
                ...
                "Rscript /opt/tools/gridss/2.8.3/gridss_somatic_filter.R -p /opt/resources/gridss_pon -i /data/output/tumor.viral_annotation.vcf -o /data/output/tumor.gridss.somatic.filtered.vcf -f /data/output/tumor.gridss.somatic.vcf -s /opt/tools/gridss/2.8.3",

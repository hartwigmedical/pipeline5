...
public class StructuralCallerTest extends StageTest<StructuralCallerOutput, SomaticRunMetadata> {
    ...
    protected List<String> expectedCommands() {
        return ImmutableList.of("export PATH=\"${PATH}:/opt/tools/bwa/0.7.17\"",
                ...
                "java -Xmx16G -cp /opt/tools/gripss/1.2/gripss.jar com.hartwig.hmftools.gripss.GripssApplicationKt -ref_genome /opt/resources/reference_genome/hg37/Homo_sapiens.GRCh37.GATK.illumina.fasta -breakpoint_hotspot /opt/resources/knowledgebases/hg37/KnownFusionPairs.hg19.bedpe -breakend_pon /opt/resources/gridss_pon/hg37/gridss_pon_single_breakend.hg19.bed -breakpoint_pon /opt/resources/gridss_pon/hg37/gridss_pon_breakpoint.hg19.bedpe -input_vcf /data/output/tumor.gridss.unfiltered.vcf.gz -output_vcf /data/output/tumor.gridss.somatic.vcf.gz",
                "gunzip -c /data/output/tumor.gridss.somatic.vcf.gz | awk '$7 == \"PASS\" || $7 == \"PON\" || $1 ~ /^#/ ' | /opt/tools/tabix/0.2.6/bgzip  | tee /data/output/tumor.gridss.somatic.filtered.vcf.gz > /dev/null",
                "/opt/tools/tabix/0.2.6/tabix /data/output/tumor.gridss.somatic.filtered.vcf.gz -p vcf");
    }
